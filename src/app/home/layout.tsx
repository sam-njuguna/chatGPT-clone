...
const layout: FC<layoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex relative w-full h-full z-0 supports-[min-height:100dvh]:min-h-[100dvh]">
      ...
      <Shortcuts />
      {isKeyModal && (
        <div className="h-screen right-0 left-0 w-full flex justify-center items-center fixed inset-0 bg-gray-300/60 dark:bg-gray-600/60 z-40">
          <Keys />
        </div>
      )}
      {isSettingModal && (
      ...
      {isCustomModal && (
        <div className="min-h-screen overflow-y-scroll  py-6 right-0 left-0 w-full flex justify-center items-center fixed inset-0 bg-gray-300/60 dark:bg-gray-600/60 z-40">
          <Customize />
        </div>
      )}
      {isUpgradeModal && (
